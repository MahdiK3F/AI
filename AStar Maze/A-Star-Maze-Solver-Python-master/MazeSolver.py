...
file_infos = {"map": [], "startPos": {"x": "", "y": ""}, "endPos": {"x": "", "y": ""}, "Fx": [], "ManX": [], "Gx": [], "amountSteps": "", "reverseWinningPath": []}
...
#Function that initializes the Gx map
def createGx(a):
    Gx = []
    ...
    for line in range(0, len(file_infos["map"])):
        ...
        for case in range(0, len(file_infos["map"][line])):
            ...
            #Insert "alpha" * the value of "Fx" at the starting position in "Gx"
            if case == file_infos["startPos"]["x"] and line == file_infos["startPos"]["y"]:
    ...
    #Store the Gx map in the "Gx" proprety of file_infos
    file_infos["Gx"] = Gx
...
def moveAround(a):
    ...
    def getLeft():
        if file_infos["map"][minimum["y"]][minimum["x"] - 1] != 1.0 and file_infos["Gx"][minimum["y"]][minimum["x"] - 1] == False:
            ...
            file_infos["Gx"][minimum["y"]][minimum["x"] - 1] = (a * file_infos["ManX"][minimum["y"]][minimum["x"] - 1]) + (file_infos["Fx"][minimum["y"]][minimum["x"] - 1])
    ...
    def getUp():
        if file_infos["map"][minimum["y"] - 1][minimum["x"]] != 1.0 and file_infos["Gx"][minimum["y"] - 1][minimum["x"]] == False:
            if file_infos["Fx"][minimum["y"]][minimum["x"]] == - 1:
            ...
            file_infos["Gx"][minimum["y"] - 1][minimum["x"]] = (a * file_infos["ManX"][minimum["y"] - 1][minimum["x"]]) + (file_infos["Fx"][minimum["y"] - 1][minimum["x"]])
    ...
    while not file_infos["Gx"][file_infos["endPos"]["y"]][file_infos["endPos"]["x"]]:
        ...
        for line in range(0, len(file_infos["map"])):
            ...
            for case in range(0, len(file_infos["map"][line])):
                ...
                if isinstance(file_infos["Gx"][line][case], float):

                    #If we don't already have a minimum value, take the first one we find
                    if minimum["value"] == "":
                        minimum.update({"x": case, "y": line, "value": file_infos["Gx"][line][case]})

                    #If we already have one, check if the value is smaller that our minimum
                    elif file_infos["Gx"][line][case] < minimum["value"]:
        ...
        file_infos["Gx"][minimum["y"]][minimum["x"]] = True
...
def trackBack():
    ...
    while True:
        ...
        #If  currentposition is not on first row
        if currentPosition["y"] != 0 and file_infos["Gx"][currentPosition["y"] - 1][currentPosition["x"]]:
        ...
        if currentPosition["x"] != 0 and file_infos["Gx"][currentPosition["y"]][currentPosition["x"] - 1]:
        ...
        if currentPosition["x"] < (len(file_infos["map"][0]) - 1) and file_infos["Gx"][currentPosition["y"]][currentPosition["x"] + 1]:
        ...
        if currentPosition["y"] < (len(file_infos["map"]) - 1) and file_infos["Gx"][currentPosition["y"] + 1][currentPosition["x"]]:
